...
\TODO{Should explain what is Map.}and reduces the tasks and reports the status to Job Tracker. Job Tracker tracks the tasks, schedules the jobs and monitors the Tasks Tracker. As you clearly see,
\TODO{We don't talk like this in a paper.}Job Tracker performs multiple operations which is bottleneck as we keep increasing the number of Task Trackers. Maximum 4000 Task Tracker and 40,000 concurrent tasks, can be handled by single Job Tracker. This was causing the limitation on use of number of Tasks Trackers. In MapReduce, there was a hard partition on resource utilization, which was causing inefficient use of resources \cite{www-5}.
...
\end{figure}
\TODO{The figure title is inappropriate.}
...
\TODO{What is the full name for ISV?}engines with a consistent framework which allows developer to write data access applications that run in Hadoop.
...
\TODO{Once again, the figure title shouldn't be "source".
Also, when you place a figure within your paper without introducing it in your
paper, why do you put it here?}
...
\begin{figure}[htbp]
	...
	\label{fig:false-color}
\end{figure}
\TODO{Another wrong figure title. Also, please describe the figure within the
text.}
...
\end{itemize}
\TODO{Generally, bullet point is not welcome in paper.}
\section{Feature}
...
\TODO{Use case on Yarn is missing.}
