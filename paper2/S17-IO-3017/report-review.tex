...
\TODO{Abstract looks good, but can be expanded a little bit. One
  sentence on each: 1) Why are graph databases like Facebook Tao
  necessary; what kind of data do they work on (give a couple
  real-world examples)? 2) What does Facebook Tao bring to the table
  that other graph databases like Neo4j don't?}

\TODO{You did a great job with this paper. The biggest improvement you
  can make is add a Conclusion that summarizes the strengths of TAO
  and what it allows Facebook to do at great scale. In addition, a
  section on comparing TAO to other graph databases like Neo4j would
  be very useful if you can find some research on that. In addition,
  see below for some other minor improvements you can make.}

\TODO{Assessment: Minor improvements suggested.}
...
\TODO{Good intro and motivation, with one possible improvement. It
  could be written in a less historic voice, and more present
  voice. E.g. instead of "As early as 2005, Facebook started using
  MySQL, a ... Facebook soon realized.", you can write "Graph
  databases like Facebook Tao are a response to data needs that
  traditional RDBMSs like MySQL do not meet. For example, Facebook..."
  and so on. This is what you find in most scientific papers, because
  they try to be as independent of a particular point in time as
  possible.}
...
\TODO{The paper will flow better if you focus on the topic rather than saying "in this paper..." That is, the first part of your sentence can be omitted and you can simply start "Facebook Tao (The Association and Objects) is a geographically distributed data store tha provides... \cite{book-tao1}"}
...
Lastly \TODO{,} high read availability for the same reasons mentioned above, which is achieved by using alternate data sources. \TODO{Not a complete sentence.}

\TODO{Good introduction that gives enough background and motivation, but doesn't go out of scope.}
...
For every association type a so-called inverse type can be specified. Whenever an edge of the direct type is created or deleted between objects with unique IDs id1 and id2 \TODO{When you're using names like $id1$ and $id2$, wrap them in \\emph or math mode to distinguish them from regular text.}, TAO will automatically create or delete an edge of the corresponding inverse type in the opposite direction (id2 to id1). The intent is to help the application programmer maintain referential integrity for relationships that are naturally mutual, like friendship, or where support for graph traversal in both directions is performance critical, as for example in “likes” and “liked by."

\TODO{Good discussion of figure.}
...
This section describes the architecture of TAO. \TODO{Unnecessary sentence.} TAO is separated into layers: two caching layers and a storage layer.
...
\TODO{Good but please discuss how this works with the distributed nature of TAO. I.e. is MySQL installed on every node? How is this decision affected by the earlier discussion you had on the limitation of RDBMSs? Etc.}
...
\TODO{Please, use the standard convention for references where they are added at the end of a statement.}
