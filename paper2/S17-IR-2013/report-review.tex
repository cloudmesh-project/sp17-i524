...
\TODO{why empbasize "the early days"? anything has changed now?}
...
OpenCV was designed to be a cross-platform tool, due to which it is completely written in C. This makes it portable to any computer system, right from PCs, Macs, to robotic on board computers. \TODO{justify your claim here, either by citing or explain why C makes it portable? The octet length of INT are not even the same across C compilers on different platform.} Since OpenCV 2.0 version, there is a C and C++ interface also, and all new packages are written in C++. However, to encourage widespread use, wrappers for popular programming languages like Python and Java have been developed and OpenCV can be used both on mobile and desktop. We look at the latest additions to OpenCV platforms \cite{www-platforms}:
...
OpenCv\TODO{OpenCV} requires numpy and matplotlib (optional) packages to be installed in a Python 2.7 environment to be able to run successfully. After making sure that the dependencies are installed, the following steps will help to setup OpenCV for Python \cite{www-opencv-python}:
...
import cv2
\end{Verbatim}
\TODO{why you choose python as an example here? it doesn't seem sufficient, if OpenCV has covered many different platforms with many languages available.}
...
\TODO{we have a few projects choose to use OpenCV, give them a helloworld to illustrate the basic parts consisting an OpenCV enabled program.}
