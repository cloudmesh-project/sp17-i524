...
\TODO{
  \TE Abstract - Don't use the term this paper.
}
...
\TODO{
  \GE
  Grammar :developers, offers, etc.
  Reference the figures properly. Please use labels in figures and use ref tag to reference the images. 
}
...
\item Projects: 
  ...
  \TODO{
    \TE
    Use few bulletings, use more paragraphs to convey the ideas. 
}
...
\TODO{
  \TE
  : not appropriate.
  use a, an where it is appropriate. 
}
...
\TODO{
 \GE Grammar : use comma appropirately. 
}
...
\item Regional storage:
  ...
  \TODO{
    \TE
    Don't use 'you want', just elaborate the idea in non-subjective format. 
}
...
\item Nearline Storage:
    ...
    \TODO{
      \GE
      Check singular calls in statements. 
}

    \TODO{
      \TE
      Don't use 'you' just explain it without talking to someone.
      Use passive voice where it is necessary without the subject.
      It brings formality to the document. 
}
...
IAM grants access to project's bucket and objects.For example, you can specify that a user has full control of all the objects in your project, but cannot create, modify, or delete any buckets in your projects. IAM gives broad control over the project but not fine-grained control over individual buckets or projects.
  ...
  \TODO{
      \TE
      Don't use 'you' or 'we' just explain it without talking to someone.
      Use passive voice where it is necessary without the subject.
      It brings formality to the document.
      'It is an authorization mechanism offered by GCS in which read or write access is granted for individual buckets or objects.'
      This way is more suitable for a paper. 
...
In the cloud platform business where all the market leader like AWS, Microsoft Azure, IBM Bluemix etc., Google cloud platform is also giving good competition.It is spreading its services by coming up with more regions and availability zones all across the world with cheap rates. 
  ...
  \TODO{
      \TE
      Avoid using 'this paper'. -> In reference to the facts discussed or mentioned would be better. 
}
