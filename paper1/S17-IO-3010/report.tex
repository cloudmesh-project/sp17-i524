...
\TODO{This review document is provided for you to achieve your
  ...
  all possible improvement suggestions and if you see comment you may
...
sentences of the abstract are totally inappropriate with the abstract.} \ML

\section{Introduction}

Couchbase, Inc. offers Couchbase Server (CBS) to the marketplace as its entry in the NoSQL, \textit{big data} database field \CE. \TODO{When you mention a technology for the first time in a paper, that's where you should cite.}  Salient features include a) an integrated cache tier which is essentail \SE \ML to the product's operation; b) persistent storage in JSON document format, i.e. document-based storage, or simple key-value pairs; c) relatively uncomplicated scalability across clusters of commodity servers; d) sub-millisecond response times; e) a SQL-like query language; and, f) built-in cluster replication, failover and disaster recovery features. In addition, Coucbhase markets a mobile product, Couchbase Mobile, which uses a Couchbase-designed syncing system to extend CBS to mobile devices and offline use cases.

\TODO {I don't see the point putting company history in a technology review.} \ML
...
least one cluster of interconnected servers \TE \ML
\TODO{weird, server computers?} \ML
running a copy of CBS. By
default, the CBS system's individual computers \TE \TODO{same here, CBS system computers?} \ML,
referred to as nodes, work together in a
master-master setup, which Couchbase calls a peer-to-peer topology. \CE  In a
master-master distributed cluster, the nodes co-exist in flat hierarchy \SE \ML, i.e.,
...
In addition, the database administrator \SE \ML can override the default peer-to-peer
topology by taking advantage of CBS' \textit{multi-dimensional scaling}.  This
functionality allows the administrator \SE \ML to customize nodes to perform tasks for
which the node is best suited, e.g., memory-intensive processes or
I/O-intensive processes, etc.

\TODO{Font size within the figure is too small to read.} \ML
...
compatible software development kits, also known as \textit{devkits} or \textit{SDKs}. \CE
\TODO{RESPONSE: I am not sure how to comply because this paragraph represents a portion of the mosaic of knowledge I now possess regarding CBS.  That is, it is an original thought.}
...
\textit{Spatial Views} and \textit{Global Secondary Indexes} or GSIs \cite{www-queries-cbsinc}. \CE \ML
...
in service of, its SQL-like query service.
...
complex data processing and custom data reshaping is necessary \cite{www-viewsindexing-cbsinc}."  \CE \ML Spatial View indexes allow users to create "multidimensional bounding box queries for location aware applications." 
\cite{www-viewsindexing-cbsinc}
...
provides query flexibility and speed between the key-value \SE \ML API and the Views
API.  Couchbase calls its newest method N1QL (pronounced \textit{nickel})
\TODO{mentioned in previous paragraph} \ML in
...
\TODO{This is very confusing since the difference of B+trie and B+tree is not explain at all.} \ML
...
\TODO{RESPONSE: I do not understand the critique.  The name of the tool is cbc.  I also provide a brief explanation.}
...
\TODO{what is cbq?} \TODO{RESPONSE: I do not understand the critique.  The name of the tool is cbq.  I mention its primary purpose.} CBS includes a number of other command line tools to accomplish various tasks \cite{www-cli-intro-cbsinc}. \TE \ML
\TODO{CLI, command line interface. No one says command line interface interfaces.} 
...
the case.} \ML
\paragraph{Graphical Interface} Couchbase implements CBS' GUI \TE \ML
\TODO{GUI, graphical user interface, needs to be in capitals.} \ML via a web
browser.  Users access the web GUI by navigating to a cluster's url appended
with the admin port number.  The browser interface acts as the
primary management tool for CBS.
\TODO{Why are you put the actual port number in your paper? Are you trying to
create a security breach?} \TODO{RESPONSE: Couchbase openly publishes the admin port number, so mentioning it did not pose a security threat.}  It offers access to node management, queries,
indexes, etc. \cite{www-webconsole-cbsinc}


\TODO{The figure is confusing here. ...} \ML \TODO{Figure removed}
\section{Licensing}
