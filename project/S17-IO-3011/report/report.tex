...
\TODO{Scott McClary: need to update the plan as timing information
...
research group.  \TODO{Working to figure out where I will store the
  data and how to partition the data amongst available VMs.}
...
validate as well). \TODO{Scott McClary: need to continue working to
  connect to IU's VPN (and DUO) from the command line. So far this
  solution is not yet complete. Need to figure out licensing
  information for Jarrold Group code.}
...
required to run the application.  \TODO{Add timing information for one
  or more clouds (i.e. Chameleon) to Figure \ref{fig:scalability2} and
  compare the results (if possilbe).}
