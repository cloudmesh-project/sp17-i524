...
\ociscodes{Docker, Ansible, Kubernetes, R, Spam}
...
libraries. The cluster management tool - \emph{Kubernetes}, is used to
auto deploy the application on a multi-node cluster. Scripts are
written in \emph{Ansible} \cite{www-ansible} to automate the
deployment process. As Kubernetes itself scales the application
container according to our specification, a deployment engineer shall
only be responsible to write a yaml specification file that is read by
the kubernetes engine. Application benchmarking results are achieved
by running CMD5 commands that trigger ansible scripts to deploy the
application on Chameleon cloud and Kilo cloud.


\section{ELEMENTS OF THE PROJECT}

\subsection{Ansible}
Computer professionals prefer automation over repetitive tasks. With
Ansible, IT admins can begin automating away the drudgery from their
daily routine tasks. Ansible is an open source automation engine that
can perfectly describe an IT application infrastructure and can be
used to automate cloud provisioning, configuration management, and
application deployment. It can also perform more advanced IT tasks
such as continuous deployment or rolling out updates with zero
downtime. A major difference in Ansible and many other tools in the
...
machine and other being the nodes. Ansible uses a single controlling
...
\noindent
Ansible modules are deployed over SSH. These modules are
...
\emph{Playbooks} are a powerful mode of using Ansible’s configuration,
deployment, and orchestration capabilities
\cite{www-ansible-playbook}. They let us control the remote systems
with a policy which we might want them to enforce. Playbooks can be
used to manage configurations of and deployments to remote
machines. They can sequence multi-tier rollouts involving rolling
updates, and can delegate actions to other hosts, interacting with
...
more components it consists of and thus requires organized  management of these
...
Since the output produced by our application spans over multiple lines
we shall redirect and save the output to a file so that it can be
referenced at any point of time with ease.
\begin{figure}[H]
\begin{verbatim}
...
Ansible along with the python's CMD5 module \cite{www-cmd5}.Python's
...
To install CMD5 module, there are couple of ways to do it but the most
...
following 3 repostories. It is mandatory to have Git \cite{www-git}
already installed.
...
\noindent
After cloning these repositories, you may enter each repository and
...
\end{verbatim}
\caption{setting up the repo}
\label{running the setup for the repo}
\end{figure}
...
$ pip install .
\end{verbatim}
\caption{installing the repo}
\label{Installing the repo}
\end{figure}
...
create your own command. It also registers it dynamically with
cloudmesh. Later, you should cd in the newly created directory and
execute \newline \$ python setup.py install \newline \$ pip install
. \newline This will successfully install the newly created command
with the CMD5 module.
...
This command assigns a name to the cluster given in <nameOfTheCluster>
placeholder. When the cluster is deployed, it will create a cluster
with this name on the cloud.
...
<sizeOfCluster> placeholder. For example, Kubernetes size 2 would
create a cluster with 2 instances on the specified cloud.
...
define any valid image for the kilo cloud as well. Important thing
here is that whichever cloud you might define for the cluster, it
should match with its equivalent valid image.
...
we are allocated an instance with the desired space.
...
above on the cloud provided in the cloudname placeholder. For example,
Kubernetes cloud chameleon. This would create the cluster on chameleon
cloud when the kubernetes cluster deploy command will be executed.
...
commands. This would help in giving you all the details for your
kubernetes cluster. This command would print out the name of the
cluster, its size, the image and the flavor used and on which cloud on
the console itself.
...
kubernetes master instance. Running the pod makes the execution of the
spam detection application and its gives the output after some
stipulated time.
...
\noindent
For our Kubernetes project, we have created 5 yml files called as
ansible playbook which would be executed to perform tasks on the
remote cloud cluster. These files contain the host name and the roles
in it.\newline

\noindent
The first playbook which is triggered when the CMD5 kubernetes cluster
deploy command is executed is the installations.yml. This playbook
does initial cleaning of previously present files if any on the
cluster, installs git and performs an update operation on all the
instances in the cluster.\newline

\noindent
Next up is kubernetes.yml playbook which has the setup role which sets
up kubernetes in all the instances in the cluster. \newline

\noindent
The third playbook to be executed is master.yml playbook. It
initializes one of the cluster as master instance in the kubernetes
cluster and initializes the kubernetes cluster.\newline

\noindent
After the master is up and running, the slaves.yml playbook is
executed which connects the rest of all the instances to the master
instance. It takes the master's ip address and token generated and
executes the join command in kubeadm library. The first 4 playbooks
are part of the Kubernetes deploy command.\newline

\noindent
Last is the runningapplicationonkubernetes.yml playbook which executes
the spam detection application on one of the slave which is
dynamically chosen and performs benchmarking of the time taken by
various algorithms in classifying an email as spam or valid.
...
\begin{minipage}[t][3.2cm][t]{1.0\columnwidth} % Adjust height [3.2cm]
                                               ...
                                               % photos.
  \begin{wrapfigure}{L}{0.25\columnwidth}
    \includegraphics[width=0.25\columnwidth]{images/sagar_vora.png}
  \end{wrapfigure}
  \noindent
