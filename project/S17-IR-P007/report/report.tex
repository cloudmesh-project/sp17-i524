...
\dates{Project-S17-IR-P007, \today}
...
\say{\emph{Playbooks} are a powerful mode of using Ansible’s
...
\noindent
The \say{ansible-playbook} command followed by the playbook name is
used to execute the playbook.  The \say{-i} option specifies the
...
necessary repositories and install the setup file to get the functionality of
CMD5 up and running.
...
\noindent
This command would create a CMD5 command with the name provided as an argument. It downloads a template from
...
create the new command. It also registers it dynamically with
cloudmesh. To finish we need to move into the newly created directory and
...
created our own customized kubernetes command called as
...
\noindent
We have created 8 Kubernetes CMD5 sub-commands and those commands trigger
